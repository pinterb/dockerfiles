...
## Start PINTER's Stuff
#
# VAULT_CACERT is typically set by client applications.  If set here, then
# the cert is added to the server's ca-certificates configuration.
if [[ -n "$VAULT_CACERT" && -f "$VAULT_CACERT" ]]; then
  cp "$VAULT_CACERT" /usr/local/share/ca-certificates/
